    ...
    for i in range(end-start):
        ...
        for j, name, parent in zip(range(len(names)), names, parents):
            ...
            if pm.hasAttr(node, 'rotate'):
                eulers[i, j] = np.radians(
                    pm.xform(node, q=True, rotation=True))
        ...
        positions[i, 0] = pm.xform(
            root, q=True, translation=True, worldSpace=True)

    rotations = orients[np.newaxis] * \
        Quaternions.from_euler(eulers, order='xyz', world=True)
